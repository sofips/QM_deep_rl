    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            ...
            with tf.variable_scope('l15', reuse=self.rus):
                w15 = tf.get_variable('w15', [n_l1, n_l1/3], initializer=w_initializer, collections=c_names, trainable=trainable)
                b15 = tf.get_variable('b15', [1, n_l1/3], initializer=b_initializer, collections=c_names, trainable=trainable)
                l15 = tf.nn.relu(tf.matmul(l1, w15) + b15)

            with tf.variable_scope('l2', reuse = self.rus):
                w2 = tf.get_variable('w2', [n_l1/3, self.n_actions], initializer=w_initializer, collections=c_names,  trainable=trainable)
        ...
        with tf.variable_scope('eval_net', reuse = self.rus):
            c_names, n_l1, w_initializer, b_initializer = \
                ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 120, \
